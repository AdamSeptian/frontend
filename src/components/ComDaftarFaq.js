...
const ComDaftarFaq = () => {
  ...
  const [searchOption, setSearchOption] = useState("judul");
  ...
  const deleteFaq = async (faqId) => {
    try {
      await axios.delete(`http://localhost:5000/faq/${faqId}`);
      fetchFaq();
    } catch (error) {
  ...
  const filterSearch = sortedFaq.filter((item) => {
  ...
  return (
    <div>
      <Head title={"E-Caku | Daftar FAQ"} />
      <Sidebar>
        <div className="md:flex gap-2 mb-4">
          <h1 className="text-5xl font-semibold mb-4 md:mb-0 w-full">
            Daftar FAQ
          </h1>
          ...
          <select
            ...
            className="select select-bordered w-full md:max-w-32"
          >
            <option value="judul">Judul</option>
            <option value="deskripsi">Deskripsi</option>
          </select>
        </div>
        <Link to="/buat-faq" className="btn btn-purple mb-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="overflow-x-auto mb-4">
          <table className="table">
            ...
            <tbody>
              {currentPosts.map((faq, index) => (
                <tr key={index} className="text-justify text-wrap">
                  <td>{index + 1}</td>
                  <td>{faq.judul}</td>
                  <td>{faq.deskripsi}</td>
                  ...
                  <td className="flex justify-center gap-2">
                    ...
                    <div className=" tooltip" data-tip="Hapus">
                      <button
                        onClick={() => confirmDelete(faq.uuid)}
