...
const ComDaftarPengguna = () => {
  ...
  const filterSearch = sortedUsers.filter((item) => {
    return (
      item[searchOption].toLowerCase().includes(searchItem.toLowerCase())
    );
  });
  ...
  return (
    <div>
      <Head title={"E-Caku | Daftar Pengguna"} />
      <Sidebar>
        <div className="md:flex gap-2 mb-4">
          <h1 className="text-5xl font-semibold mb-4 md:mb-0 w-full">Pengguna</h1>
          ...
          <svg
            ...
            >
              <option value="username">Nama Pengguna</option>
              <option value="email">Email</option>
              <option value="role">Role</option>
            </select>
        </div>
        <Link to="/buat-pengguna" className="btn btn-purple mb-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="overflow-x-auto mb-4">
          <table className="table">
            ...
            <tbody>
              {currentPosts.map((pengguna, index) => (
                <tr key={index} className="text-center text-wrap">
                  <td>{index + 1}</td>
                  <td>{pengguna.username}</td>
                  <td>{pengguna.email}</td>
                  <td>{pengguna.role}</td>
                  <td>{formatDate(pengguna.createdAt)}</td>
                  <td>{formatDate(pengguna.updatedAt)}</td>
                  <td className="flex justify-center gap-2">
                    <div className=" tooltip" data-tip="Edit">
                      <Link
                        to={`/daftar-pengguna/edit/${pengguna.uuid}`}
                    ...
                    <div className=" tooltip" data-tip="Hapus">
                      <button
                        onClick={() => confirmDelete(pengguna.uuid)}
...
export default ComDaftarPengguna;
